...
let LoginController = class LoginController {
    ...
    async resetPassword(req, auth, currentUser) {
        ...
        if (req.oldPassword) {
            ...
            if (process.env.PRIVATE_DECRYPTION_KEY) {
                const decryptedOldPassword = await this.userRepo.decryptPassword(req.oldPassword);
                const decryptedNewPassword = await this.userRepo.decryptPassword(req.password);
                oldPassword = decryptedOldPassword;
                newPassword = decryptedNewPassword;
            }
            changePasswordResponse = await this.userRepo.updatePassword(req.username, oldPassword, newPassword);
        }
        else {
            let newPassword = req.password;
            if (process.env.PRIVATE_DECRYPTION_KEY) {
                const decryptedPassword = await this.userRepo.decryptPassword(req.password);
                newPassword = decryptedPassword;
            }
            changePasswordResponse = await this.userRepo.changePassword(req.username, newPassword);
        }
